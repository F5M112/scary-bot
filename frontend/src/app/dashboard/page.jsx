...
Crown, Plus, Link as LinkIcon, Unlink, AlertCircle,
} from 'lucide-react';
...
try {
  ...
  toast.success(`✅ ${t('serverRegistered')} ${g.name}`);
...
registeredGuilds.map((g) => g.guildId)
);
    ...
    {registeredGuilds.length === 0 ? (
      ...
      <div className="grid md:grid-cols-2 gap-4">
        ...
        {registeredGuilds.map((g) => (
          <div
            key={g.guildId}
            className="card hover:border-brand-500/40 transition flex items-center gap-4 group"
          >

            <div className="w-14 h-14 rounded-xl bg-brand-600/20 flex items-center justify-center overflow-hidden shrink-0">
              {g.guildIcon ? (
                <img
                  src={`https://cdn.discordapp.com/icons/${g.guildId}/${g.guildIcon}.png`}
                  alt={g.guildName}
            ...
            <Link
              href={`/dashboard/server/${g.guildId}`}
