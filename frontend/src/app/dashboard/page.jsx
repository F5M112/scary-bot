...
export default function DashboardHome() {
  ...
  useEffect(() => {
    ...
    if (err) {
      const messages = {
  already_linked: t('discordAlreadyLinked'),
  invalid_state:  t('discordSessionExpired'),
  oauth_failed:   t('discordOAuthFailed'),
  missing_params: t('discordMissingParams'),
};
      ...
      if (user?.isDiscordLinked) {
        try {
          ...
          if (dis.data.tokenExpired) {
            toast.error(t('discordLinkExpired'));
    ...
    <div className="space-y-8">
      <div>
        ...
        <div className="flex items-center gap-2 text-sm">
          <span className="text-white/60">{t('currentPlanLabel')}:</span>
          {user?.plan === 'premium'
      ...
      {!user?.isDiscordLinked ? (
        <div className="card bg-gradient-to-br from-[#15090a] to-blue-950/20 border-blue-500/30">
          <div className="flex items-start gap-4">
            ...
            <div className="flex-1">
              <h3 className="font-bold mb-1">{t('linkDiscordTitle')}</h3>
        ...
        <div className="card flex items-center gap-4 flex-wrap">
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="text-xs text-white/50">{t('discordAccountLinked')}</div>
          </div>
      ...
      {/* Registered Servers */}
      <section>
        <h2 className="text-xl font-bold mb-4">{t('managedServers')}</h2>
        {registeredGuilds.length === 0 ? (
          <div className="card text-center py-12">
            <Server className="mx-auto text-white/30 mb-4" size={48} />
            <p className="text-white/60">{t('noRegisteredServers')}</p>
  ...
  {user?.isDiscordLinked
    ...
    : t('linkDiscordFirst')}
      ...
      {user?.isDiscordLinked && availableGuilds.length > 0 && (
        <section>
          <h2 className="text-xl font-bold mb-2">{t('availableServers')}</h2>
          <p className="text-sm text-white/50 mb-4">{t('serverPermissionDesc')}</p>
  ...
  {g.owner ? `👑 ${t('owner')}` : `🛡️ ${t('admin')}`}
      ...
      {user?.isDiscordLinked && discordGuilds.length === 0 && (
        <div className="card bg-amber-500/5 border-amber-500/20 text-center py-8">
          ...
          <p className="text-white/70">{t('noAdminServers')}</p>
        </div>
      )}
    </div>
  );
